...
next_page = tk.BooleanVar()#make sure the scrip continues to run only when the 'Next' Button is clicked
next_page.set(False)
...
def UploadAction():
    filename = filedialog.askopenfilename(filetypes = (("Excel files", ".csv"),))#only select csv files
...
input_button = tk.Button(page1, text='Select Input Table',width=20,
                          height=4, command=UploadAction)
...
page2_button = tk.Button(page1, text='Next',width=8,
                          height=2, command=Close_P1, state = 'disabled')
...
#the script will not continue if the current page was closed by clicking the cross on right top
...
#the 'Next' button
page3_button = tk.Button(page2, text='Next',width=8,
                          height=2, command=Close_P2, state = 'normal')
...
for i in range(len(data_columns)):
    ...
    tk.Radiobutton(page2,text=col_name,variable=y_selection,value=i).grid(row=placement,column=10,sticky="w")
    placement+=10
...
#the 'Next' button
page4_button = tk.Button(page3, text='Next',width=8,
                          height=2, command=Close_P3, state = 'normal')
...
for i in range(len(var_types)):
    ...
    tk.Radiobutton(page3,text=variable_type,variable=ytype_selection,value=i).grid(row=placement,column=10,sticky="w")
    placement+=10
...
page5_button = tk.Button(page4, text='Next',width=8,
                          height=2, command=Close_P4, state = 'normal')
...
for i in range(len(X)):
    ...
    tk.Checkbutton(page4,text=predictor,variable=selected_var,onvalue=1, offvalue=0).grid(row=placement,column=10,sticky="w")
    row = []
    ...
    for j in range(len(var_types)):
        ...
        type_button = tk.Radiobutton(page4,text=variable_type,variable=type_of_predictor,value=j, state = 'active')
...
for i in range(len(visualizations)):
    ...
    # selected_vis.set(1)
    tk.Checkbutton(page5,text=vis_plot,variable=selected_vis,onvalue=True, offvalue=False).grid(row=placement,column=10,sticky="w")
...
page6_button = tk.Button(page5, text='Run',width=8,
                          height=2, command=Close_P5, state = 'normal')
